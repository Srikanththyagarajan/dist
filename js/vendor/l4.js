    ...
    .directive("recentBlogPosts", ["fetchBlog", "$location", function (fetch, $location) {
        return {
            ...
            link: function (scope) {
                ...
                var getFirstBlock = function (blogData) {
                    var firstBlock = [], blgCount = blogData.length, maxLimit = 0;

                    if (typeof (blogData) != "undefined") {
                        maxLimit = blgCount > 3 ? (blgCount < 3 ? blgCount > 3 : 3) : blgCount;
                        for (var i = 0; i < maxLimit; i++) {
                            firstBlock.push(blogData[i]);
                        }
                    }

                    //angular.forEach(blogData, function (blog, index) {
                    //    if (index < 3) {
                    //        firstBlock.push(blog);
                    //    }
                    //});
                    return firstBlock;
                }

                var getSecondBlock = function (blogData) {
                    var secondBlock = [], blgCount = blogData.length, maxLimit = 0;;

                    if (typeof (blogData) != "undefined") {
                        maxLimit = blgCount > 6 ? 6 : (blgCount > 3 ? blgCount : 0);
                        for (var i = 3; i < maxLimit; i++) {
                            secondBlock.push(blogData[i]);
                        }
                    }

                    //angular.forEach(blogData, function (blog, index) {
                    //    if (index > 2) {
                    //        secondBlock.push(blog);
                    //    }
                    //});
                    return secondBlock;
                }
                ...
                fetch.getBlog(searchParam).then(function (data) {
                    ...
                    scope.firstBlock = getFirstBlock(data.Blog);
                    scope.secondBlock = getSecondBlock(data.Blog);
                });
